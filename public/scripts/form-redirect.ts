 ...
 */
export function resolveFormRedirect({
  redirectUrl,
  safeInternalHash,
  currentOrigin,
  currentPathname,
  ...
  try {
    ...
    const normalizedAllowedPrefixes = new Set<string>();

    const addNormalizedPath = (path: string) => {
      normalizedAllowedPaths.add(normalizePathSegments(path).toLowerCase());
    };
    const addNormalizedPrefix = (prefix: string) => {
    ...
    const isAllowedPath =
      normalizedAllowedPaths.has(normalizedPath) ||
      Array.from(normalizedAllowedPrefixes).some((prefix) =>
