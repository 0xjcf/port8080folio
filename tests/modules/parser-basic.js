...
const obj = {
  ...
  validateStrings(outputs) {
    ...
    Object.entries(outputs).forEach(([version, html]) => {
      ...
      const stringPatterns = [/'Hello World'/, /'secret'/, /'value'/];

      stringPatterns.forEach((pattern, _index) => {
  ...
  validateComments(outputs) {
    ...
    Object.entries(outputs).forEach(([version, html]) => {
      ...
      const commentPatterns = [/\/\/ Arrow functions/, /\/\/ Regular functions/];

      commentPatterns.forEach((pattern, _index) => {
