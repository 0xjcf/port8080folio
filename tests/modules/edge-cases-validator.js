...
class EdgeCasesValidator extends window.BaseSyntaxValidator {
  ...
  validateTemplateLiterals(content) {
    ...
    if (hasTemplateLiterals) {
      const hasExpressionHighlighting =
        content.includes('${</span>') || (content.includes('<span') && content.includes('${'));
  ...
  validateQuoteHandling(content) {
    ...
    const quoteTypes = ['"', "'", '`'];
    const hasQuoteHighlighting = quoteTypes.some(
      (quote) => content.includes(`>${quote}`) || content.includes(`${quote}<`)
    );

    this.addValidation(
      'Quote Type Highlighting',
  ...
  validateXStateEdgeCases(content) {
    ...
    const hasXStateKeywords =
      content.includes('createMachine') ||
      content.includes('context') ||
      content.includes('states');

    this.addValidation(
      'XState Keywords in Edge Cases',
      hasXStateKeywords,
