...
class BaseSyntaxValidator {
  ...
  validateHTMLEntityEscaping(content) {
    ...
    if (hasTemplateLiterals) {
      const hasProperTemplateLiteralStructure =
  ...
  validateControlFlowKeywords(content) {
    ...
    if (hasTry && content.includes('finally')) {
      // Check if 'finally' exists in the raw text
      this.addValidation(
  ...
  validateLanguageSpecific(_content, _language) {
