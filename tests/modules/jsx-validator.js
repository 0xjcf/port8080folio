...
class JSXValidator extends window.BaseSyntaxValidator {
  ...
  validateJSXSpecific(content) {
    ...
    const hasJSXBracketHighlighting =
      content.includes('--jsx-bracket') || content.includes('jsxBracket');
    const hasJSXAttributeHighlighting =
      content.includes('--jsx-attribute') || content.includes('jsxAttribute');
    ...
    this.addValidation(
      ...
      hasJSXTagHighlighting
        ? []
        : [
            'JSX tags are NOT being highlighted! Missing --jsx-tag CSS variable. This means matchJSX() is not working.',
          ]
    );

    this.addValidation(
      'JSX Bracket Highlighting',
      hasJSXBracketHighlighting,
      hasJSXBracketHighlighting
        ? []
        : ['JSX brackets (< >) are NOT being highlighted! Missing --jsx-bracket CSS variable.']
    );
    ...
    this.addValidation(
      'JSX Elements Properly Highlighted',
      hasProperJSXElements,
      hasProperJSXElements
        ? []
        : [
            'JSX elements (like <div>, <button>) are NOT highlighted as JSX! They are being treated as plain text.',
          ]
    );
    ...
    if (
      ...
      this.addValidation(
        'JSX Pattern Recognition',
        jsxPatternsHighlighted,
        jsxPatternsHighlighted
          ? []
          : ['Code contains JSX elements but they are NOT highlighted with JSX-specific styles!']
      );
    }
    ...
    if (foundAttributes.length > 0 && hasJSXAttributeHighlighting === false) {
      this.addValidation('JSX Attribute Highlighting Applied', false, [
    ...
    this.addValidation(
      'JSX Structure Integrity',
      hasProperJSXStructure,
      hasProperJSXStructure ? [] : ['JSX structure appears broken - check for malformed tags']
    );

    // SAFEGUARD: Final check - if we have JSX code but no JSX highlighting at all, FAIL
    const hasAnyJSXHighlighting =
      hasJSXTagHighlighting || hasJSXBracketHighlighting || hasJSXAttributeHighlighting;
    if (hasJSXTags && !hasAnyJSXHighlighting) {
      this.addValidation('Critical: JSX Highlighting System', false, [
