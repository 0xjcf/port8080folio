...
const List = ({ data }) => (
  ...
  generateDebugInfo(testCase, result) {
    ...
    const highlightedDivs = (result.content.match(/style="[^"]*--jsx[^"]*"[^>]*&lt;div/g) || [])
      .length;
    const highlightedButtons = (
      result.content.match(/style="[^"]*--jsx[^"]*"[^>]*&lt;button/g) || []
    ).length;
    ...
    if (firstDiv >= 0) {
      ...
      debugInfo +=
        escapeForDisplay(result.content.substring(Math.max(0, firstDiv - 50), firstDiv + 100)) +
        '\n';
    }

    if (firstButton >= 0) {
      debugInfo += `\nFirst <button> found at position ${firstButton}:\n`;
      debugInfo += `${escapeForDisplay(
        result.content.substring(Math.max(0, firstButton - 50), firstButton + 100)
      )}\n`;
    }
