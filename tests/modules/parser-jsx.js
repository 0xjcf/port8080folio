...
function TodoApp() {
  ...
  validateJSXExpressions(outputs) {
    ...
    Object.entries(outputs).forEach(([version, html]) => {
      ...
      expressionPatterns.forEach((pattern, _index) => {
        // JSX expressions should be highlighted differently from regular braces
        const found =
          html.includes(pattern.source.replace(/[{}]/g, '')) || html.includes(pattern.source);
  ...
  validateReactHooks(outputs) {
    const reactHooks = ['useState', 'useEffect', 'useCallback', 'useMemo', 'useReducer'];
    const checks = [];
