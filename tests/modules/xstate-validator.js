...
class XStateValidator extends window.BaseSyntaxValidator {
    ...
    validateXStateSpecific(content) {
        ...
        this.addValidation('XState Keywords', 
        ...
        const hasContextHighlighting = contextRegex.test(content) || 
            (content.includes('>context<') && content.includes('var(--'));
        
        this.addValidation('Context Property Highlighting',
        ...
        const foundStateKeywords = stateKeywords.filter(keyword => 
            highlightedWords.includes(keyword) || content.includes(`>${keyword}<`)
        );
        
        this.addValidation('State Definitions',
        ...
        const xstateVars = ['--xstate-keyword', '--context-property', '--event-property', '--state-name'];
        const foundXStateVars = xstateVars.filter(varName => content.includes(varName));
        
        if (foundXStateVars.length > 0) {
            this.addValidation('XState Theme Variables',
