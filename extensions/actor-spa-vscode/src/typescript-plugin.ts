...
function init(modules: TypeScriptModule): LanguageServicePlugin {
  ...
  function create(info: ts.server.PluginCreateInfo): ts.LanguageService {
    ...
    const languageServiceProxy = new Proxy(info.languageService, {
      get(target, property) {
        const originalMethod = target[property as keyof ts.LanguageService];
    ...
    languageServiceProxy.getCompletionsAtPosition = (fileName, position, options) => {
      ...
      const frameworkCompletions: ts.CompletionEntry[] = [
        {
          name: 'send',
  ...
  function getTemplateContext(sourceFile: ts.SourceFile, position: number): TemplateContext | null {
    const tokenAtPosition = findTokenAtPosition(sourceFile, position);
    if (!tokenAtPosition) {
      return null;
    }

    let currentNode: ts.Node | undefined = tokenAtPosition;
    while (currentNode) {
      if (typescript.isTaggedTemplateExpression(currentNode)) {
        const taggedTemplate = currentNode as ts.TaggedTemplateExpression;
  ...
  function isInHtmlTemplate(context: TemplateContext): boolean {
    if (!typescript.isTaggedTemplateExpression(context.node)) {
      return false;
    }
    const taggedTemplate = context.node as ts.TaggedTemplateExpression;
