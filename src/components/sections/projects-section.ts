...
class ProjectsSection extends HTMLElement {
  ...
  private renderProjectCard(project: Project): string {
    const attributes: string[] = [
      `emoji="${this.escapeAttribute(project.emoji)}"`,
      `title="${this.escapeAttribute(project.title)}"`,
      `problem="${this.escapeAttribute(project.problem)}"`,
      `solution="${this.escapeAttribute(project.solution)}"`,
      `impact="${this.escapeAttribute(project.impact)}"`,
      `tech-stack="${this.escapeAttribute(project.techStack.join(','))}"`,
      `code-url="${this.escapeAttribute(project.codeUrl)}"`
    ];

    if (project.demoUrl) {
      attributes.push(`demo-url="${this.escapeAttribute(project.demoUrl)}"`);
    }

    if (project.blogUrl) {
      attributes.push(`blog-url="${this.escapeAttribute(project.blogUrl)}"`);
    }
  ...
  public getProjectByTitle(title: string): Project | undefined {
    return this.projects.find(project => project.title === title);
  }

  public getProjectsByTech(tech: string): readonly Project[] {
    return this.projects.filter(project => 
      project.techStack.some(stackItem => 
...
export type { Project, ProjectCardAttributes }; 