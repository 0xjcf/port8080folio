...
interface BlogPostClickEvent extends CustomEvent {
  detail: {
    title: string;
    url: string;
  };
...
class BlogPosts extends HTMLElement {
  ...
  private renderPosts(): string {
    return this.posts.map(post => this.renderPostCard(post)).join('');
  }

  private renderPostCard(post: BlogPost): string {
    return `
      <a href="${this.escapeAttribute(post.url)}" target="_blank" rel="noopener" class="post-card" data-post-title="${this.escapeAttribute(post.title)}">
        <div class="post-header">
          <span class="post-emoji">${this.escapeHtml(post.emoji)}</span>
          <div class="post-meta">
            ${post.metric ? `<p class="post-metric">${this.escapeHtml(post.metric)}</p>` : ''}
            <p class="post-readtime">${this.escapeHtml(post.readTime)} read</p>
          </div>
        </div>
  ...
  private addEventListeners(): void {
    const postCards = this.shadowRoot!.querySelectorAll('.post-card') as NodeListOf<HTMLAnchorElement>;
    
    postCards.forEach((card: HTMLAnchorElement) => {
      card.addEventListener('click', (e: Event) => {
        const title = card.dataset.postTitle;
        const url = card.href;
        
        if (title && url) {
          this.dispatchBlogPostClickEvent(title, url);
        }
      });
    });
  }

  private dispatchBlogPostClickEvent(title: string, url: string): void {
    const customEvent: BlogPostClickEvent = new CustomEvent('blog-post-clicked', {
      detail: { title, url },
      bubbles: true,
      composed: true
    });
  ...
  public getPosts(): readonly BlogPost[] {
    return this.posts;
  }

  public getPostByTitle(title: string): BlogPost | undefined {
    return this.posts.find(post => post.title === title);
  }

  public getPostsByTag(tag: string): readonly BlogPost[] {
  ...
  public async fetchBlogPosts(): Promise<void> {
    try {
      // Type-safe API integration placeholder
      ...
      // Handle error appropriately - maybe show cached posts or error state
    }
  }
...
export { BlogPosts };
export type { BlogPost, BlogPostClickEvent, BlogPostApiResponse }; 