...
interface UiStateContext {
  // Context is empty but maintaining interface for consistency and future expansion
}
...
export const createUiStateEvent = {
  orderStarted: (): Extract<UiStateEvents, { type: 'ORDER_STARTED' }> => ({ type: 'ORDER_STARTED' }),
  paymentRequested: (): Extract<UiStateEvents, { type: 'PAYMENT_REQUESTED' }> => ({ type: 'PAYMENT_REQUESTED' }),
  baristaStarted: (): Extract<UiStateEvents, { type: 'BARISTA_STARTED' }> => ({ type: 'BARISTA_STARTED' }),
  coffeeReady: (): Extract<UiStateEvents, { type: 'COFFEE_READY' }> => ({ type: 'COFFEE_READY' }),
  coffeeDelivered: (): Extract<UiStateEvents, { type: 'COFFEE_DELIVERED' }> => ({ type: 'COFFEE_DELIVERED' }),
