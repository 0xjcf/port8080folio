...
type ContentType = 'Guide' | 'Story' | 'Articles' | 'Resources' | 'Blog' | 'Demo' | 'About';
...
interface RelatedContentContext {
  currentPath: string;
  contentMap: RelatedContentMap;
  currentContent: RelatedContentData;
}
...
// Related Content Machine
const relatedContentMachine = setup({
  types: {
    context: {} as RelatedContentContext,
    events: {} as
      | { type: 'ITEM_CLICK'; item: RelatedItem }
      | { type: 'REFRESH_CONTENT' }
      | { type: 'UPDATE_PATH'; path: string },
  },
  actions: {
    updateCurrentPath: assign({
      currentPath: ({ event }) => {
        if (event.type === 'UPDATE_PATH') {
          return event.path;
        }
    ...
    updateCurrentContent: assign({
      currentContent: ({ context, event }) => {
        let path = context.currentPath;
        if (event.type === 'UPDATE_PATH') {
          path = event.path;
        } else if (event.type === 'REFRESH_CONTENT') {
          path = window.location.pathname;
        }
        return context.contentMap[path] || context.contentMap['/'];
      },
    }),
    ...
    handleItemClick: ({ event }) => {
      if (event.type === 'ITEM_CLICK') {
        ...
        if (typeof window !== 'undefined' && window.globalEventBus) {
          window.globalEventBus.emit('related-item-clicked', {
            title: item.title,
            type: item.type,
            url: item.url,
            source: 'related-content',
            timestamp: Date.now(),
            componentId: 'related-content',
          });
        }
  ...
  states: {
    idle: {
      on: {
        ...
        REFRESH_CONTENT: {
          actions: ['updateCurrentPath', 'updateCurrentContent'],
        },
        UPDATE_PATH: {
          actions: ['updateCurrentPath', 'updateCurrentContent'],
        },
  ...
  entry: ['updateCurrentPath', 'updateCurrentContent'],
});
...
const relatedItemTemplate = (item: RelatedItem) => html`
  <div 
    class="related-item" 
    send="ITEM_CLICK"
    item=${item}
...
// Main Template
const relatedContentTemplate = (state: SnapshotFrom<typeof relatedContentMachine>) => {
  const { currentContent } = state.context;
  ...
  return html`
    <div class="related-content">
      ...
      <div class="related-grid">
        ${currentContent.items.map(relatedItemTemplate)}
      </div>
...
// Create the component (unused but kept for potential future use)
const _RelatedContentComponent = createComponent({
  machine: relatedContentMachine,
  template: relatedContentTemplate,
  styles: relatedContentStyles,
  tagName: 'related-content',
});

// Export the working component
export default _RelatedContentComponent;
export type {
  ContentType,
  RelatedContentData,
  RelatedContentMap,
