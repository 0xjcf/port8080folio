...
function handleEmailPreFill(): void {
    const urlParams = new URLSearchParams(window.location.search);
    const emailParam = urlParams.get('email');

    if (emailParam) {
        // Pre-fill all email inputs on the page
        const emailInputs = document.querySelectorAll<HTMLInputElement>('input[type="email"][name="EMAIL"]');
...
const trackExternalLink = (event: Event): void => {
...
const konamiCode = ['ArrowUp', 'ArrowUp', 'ArrowDown', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'ArrowLeft', 'ArrowRight', 'b', 'a'] as const;
let konamiIndex = 0;
...
function createConfetti(color: string): void {
